...
void PlannerManager::generateTraj(vector<Vector3d> path) {
    ...
    if (mid_ret && ret) {
        ...
        sv_manager->current_robot_shape->Transform(
            planner_manager->sv_manager->lastRotate, planner_manager->recent_traj.getPos(planner_manager->recent_traj.getTotalDuration()) + Eigen::Vector3d(0, 0, 5));
